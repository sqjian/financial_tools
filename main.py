...
def reconcile_accounts(file_path, sheet_name=0):
    try:
        # 读取Excel文件
        ...
        if not all(col in df.columns for col in ["column_a", "column_b"]):
            raise ValueError("Excel文件必须包含'column_a'和'column_b'列")
        ...
        col_a = preprocess_column("column_a")
        col_b = preprocess_column("column_b")
        ...
        for match in combo_matches:
            print(
                f"组合匹配 | A行号: {match['a_rows']} | B行号: {match['b_row']} | 总金额: {match['amount']}"
            )
...
if __name__ == "__main__":
    result = reconcile_accounts("测算表.xlsx", "Sheet1")