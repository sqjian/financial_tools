...
class App(tk.Tk):
    ...
    def _create_file_loader(self):
        ...
        # 甲方文件信息标签（凹陷样式，左对齐文本）
        lbl_a = tk.Label(frame, text="甲方表的基本统计信息", relief=tk.SUNKEN, anchor=tk.W)
        lbl_a.grid(row=0, column=1, padx=5, pady=10, sticky=tk.EW)  # 横向拉伸填充

        # 乙方文件加载控件（布局同上）
        btn_b = tk.Button(frame, text="加载乙方表", width=15)
        btn_b.grid(row=1, column=0, padx=10, pady=10, sticky=tk.W)
        lbl_b = tk.Label(frame, text="乙方表的基本统计信息", relief=tk.SUNKEN, anchor=tk.W)
